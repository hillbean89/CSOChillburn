...
def wordle_wish_version(the_word):  #this function compares a users guess to the randomly selected word.  If the letter is not in the word it prints a blank spac.
    ...
    while number_of_guess>0:
        guess = str(input("Guess the word: "))#prompts the user to enter a word
        ...
        if guess==the_word:   #if they guess the word it says you guessed the word then breaks the loop and ends the program
            ...
            for c_the_word,c_guess in zip(the_word,guess):#this function goes through the_word and the guess and compares them to each other. If there are characters that match they get 
        ...
        if number_of_guess==1:  #if it is on their last guess this functions prints a hint by printing the word backwards
            print(the_word[::-1])
